...
pygame.init()
pygame.mixer.init()  # инициализатор для работы со звуком
...
"""
 скорости мяча и ракеток на каждом из уровней
"""
LEVEL_SPEEDS = {
...
SAVE_FILE = "progress.json"  # файл для сохранения прогресса игрока

screen = pygame.display.set_mode((WIDTH, HEIGHT))
pygame.display.set_caption("Пинг-Понг")
clock = pygame.time.Clock()  # объект часов для управления частотой кадров.
...
bounce_sound = pygame.mixer.Sound("ten.mp3")  # загрузка звуков
win_sound = pygame.mixer.Sound("you-win-golos.mp3")

try:
    pygame.mixer.music.load("wall_m.mp3")
    pygame.mixer.music.set_volume(0.05)  # установка громкости музыки
    pygame.mixer.music.play(-1)  # запуск музыки в бесконечном цикле
...
def draw_button(surface, color, x, y, width, height, text, font, text_color, is_enabled=True, shadow=True):
    """
    Рисует кнопку на заданной поверхности.
    """
    button_rect = pygame.Rect(x, y, width, height)
    if shadow and is_enabled:  # отрисовка тени, если включена и кнопка активна
    ...
    return button_rect, is_enabled  # возвращает прямоугольник кнопки и её состояние


"""
загружает прогресс игры из файла progress.json
Возвращает значение ключа "unlocked_level"
если ключа нет, по умолчанию вернется уровень 1
"""
...
"""
записывается текущий прогресс в файл progress.json
json.dump() - для сохранения словаря в файл
"""
...
"""
если файл есть то удаляем его и сбрасываем прогресс
"""
...
def show_start_screen(unlocked_level):
    ...
    draw_text("Выберите уровень:", FONT_BIG, WHITE, screen, WIDTH // 2, 120)

    buttons = []  # список для хранения прямоугольников кнопок
    ...
    """
    Функция для отображения стартового экрана с
    кнопками выбора уровня и выхода.
    Ждет нажатия кнопки и возвращает выбранный уровень.
    """
    waiting = True
    selected_level = 0  # переменная для хранения выбранного уровня
    while waiting:
        clock.tick(FPS)
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                for i, (button_rect, is_enabled) in enumerate(buttons):  # перебор всех кнопок
                    if button_rect.collidepoint(mouse_x,
                                                mouse_y) and is_enabled:  # проверка, была ли нажата кнопка и активна ли она
                        if i == 3:
                            return -1
                        selected_level = i + 1
                        waiting = False
...
def show_round_screen(message):
    """
    Отображает экран с сообщением на короткое время.
    """
    screen.fill(BLACK)
...
def show_end_screen(message, return_to_menu=False):
    screen.fill(BLACK)
    draw_text(message, FONT_BIG, (255, 255, 0), screen, WIDTH // 2, HEIGHT // 4)
    if return_to_menu:  # если нужно отобразить сообщение о возврате в меню
        draw_text("Нажмите любую клавишу для возврата в меню", FONT_SMALL, WHITE, screen, WIDTH // 2, HEIGHT // 2)
    else:  # если нужно отобразить сообщение о выходе
...
class Paddle:
    ...
    def move(self, target_y):
        if self.rect.centery < target_y:  # если центр ракетки выше цели
...
class Ball:
    """
    Класс, представляющий игровой мяч.
    """
    ...
    def reset(self):
        self.rect.center = (WIDTH // 2, HEIGHT // 2)  # установка центрального положения
...
def run_game(current_level, unlocked_level):
    ...
    ball = Ball(WIDTH // 2 - 10, HEIGHT // 2 - 10, 20, ball_speed_x, ball_speed_y)  # создание мяча
    ...
    while game_running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_r:
                    return True, unlocked_level  # досрочный выход из текущего уровня

        mouse_y = pygame.mouse.get_pos()[1]
        paddle1.move(mouse_y)

        paddle2.move(ball.rect.centery)  # перемещение правой ракетки вслед за мячом
        ...
        draw_text("нажмите r для дотсрочного выхода", FONT_SMALL, WHITE, screen, WIDTH // 2, HEIGHT - 30)
...
def main():
    ...
    while running:
        ...
        running, unlocked_level = run_game(current_level, unlocked_level)  # запуск выбранного уровня
        if running:
            current_level = show_start_screen(unlocked_level)
